...
// Handle typing directly into the grid (NO input box anymore)
document.addEventListener("keydown", function (e) {
  if (!inputEnabled || currentRow >= maxAttempts) return;
  ...
  if (/^[A-Z]$/.test(key)) { 
    if (currentCol < wordLength) {
      ...
      gridCells[index].textContent = key;
      currentCol++;
    }
  } 
...
function submitGuess() {
  // Build the guess from the current row
  let guess = '';
  const startIndex = currentRow * wordLength;
  for (let i = 0; i < wordLength; i++) {
    guess += gridCells[startIndex + i].textContent.toLowerCase();
  }
...
function checkAndColorLetter(guess) {
  const answerArr = answer.split('');
  const answerDict = {};

  // Build letter counts
  for (const letter of answerArr) {
    answerDict[letter] = (answerDict[letter] || 0) + 1;
  }

  // First pass: Green (correct place)
  for (let col = 0; col < wordLength; col++) {
    if (guess[col] === answerArr[col]) {
      changeBgColor(currentRow, col, 2);
      answerDict[guess[col]]--;
    }
  }

  // Second pass: Yellow (wrong place) or Gray (not in word)
  for (let col = 0; col < wordLength; col++) {
    if (guess[col] !== answerArr[col]) {
      if (answerDict[guess[col]] > 0) {
        changeBgColor(currentRow, col, 1);
        answerDict[guess[col]]--;
      } else {
...
// Show help modal when page loads
...
function disableInput() {
  inputEnabled = false;
  document.removeEventListener("keydown", keydownHandler); // Only works if you define keydownHandler separately
}
document.addEventListener("keydown", startTwoMinuteTimer);
